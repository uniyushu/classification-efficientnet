...
from pruning.oss_and_pruning_parser import *
...
# Misc
group = parser.add_argument_group('prune parameters')
group.add_argument('--initial-checkpoint-pruned', default='', type=str, metavar='PATH',
                   help='Initialize model from this checkpoint  form the pruned model (default: none)')
group.add_argument('--initial-checkpoint_IKD', default='', type=str, metavar='PATH',
                   help='Initialize model from this checkpoint (default: none)')
group.add_argument('--use_eca', action='store_true', default=False,
...
def main():
    ...
    if args.prune:
        ...
        len_loader = int(len(loader_p) * (4 * args.batch_size) / args.batch_size_prune)
        ...
        if args.distributed:
            if has_apex and use_amp == 'apex':
                ...
                if args.local_rank == 0:
                    _logger.info("Using NVIDIA APEX DistributedDataParallel.")
                co_mod = ApexDDP(co_mod, delay_allreduce=True)
            else:
                if args.local_rank == 0:
                    _logger.info("Using native Torch DistributedDataParallel.")
                co_mod = NativeDDP(co_mod, device_ids=[args.local_rank], broadcast_buffers=not args.no_ddp_bb)
    ...
    if args.rank == 0:
        ...
        saver = CheckpointSaver(
            model=co_mod, optimizer=optimizer, args=args, model_ema=model_ema, amp_scaler=loss_scaler,
            checkpoint_dir=output_dir, recovery_dir=output_dir, decreasing=decreasing, max_history=args.checkpoint_hist)
